...
from models import User, UserPhoto, UserLanguage, Swipe, Match, BlockReport, Notification, UserPreferences, Subscription, Plan
from schemas import (
    DiscoveryProfileOut, UserPhotoOut, UserLanguageOut,
...
async def _get_effective_limits(user: User, db: AsyncSession) -> tuple[int, int]:
    plan = None
    if user.is_premium:
        ...
        plan = free_result.scalars().first()

    max_likes = settings.DAILY_LIKES_FREE
    max_super = settings.DAILY_SUPER_LIKES_FREE
    if plan and plan.swipes_per_day is not None:
        max_likes = plan.swipes_per_day
    if plan and plan.super_likes_per_day is not None:
        max_super = plan.super_likes_per_day
    return max_likes, max_super
...
async def create_swipe(
    ...
    max_likes, max_super = await _get_effective_limits(user, db)
    ...
    if req.direction in ("like", "super_like"):
        ...
        if mutual.scalar_one_or_none():
            ...
            db.add(Notification(
                ...
                title="New Match!", body=f"You matched with {target.first_name}",
                related_user_id=req.swiped_id,
            ))
            db.add(Notification(
                user_id=req.swiped_id, type="match",
                title="New Match!", body=f"You matched with {user.first_name}",

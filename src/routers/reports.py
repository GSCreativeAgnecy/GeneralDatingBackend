...
async def get_blocked_users(
    ...
    out = []
    if blocks:
        target_ids = [b.reported_id for b in blocks]
        targets_result = await db.execute(select(User).where(User.id.in_(target_ids)))
        targets_map = {u.id: u for u in targets_result.scalars().all()}
        for b in blocks:
            target = targets_map.get(b.reported_id)
            if target:
                out.append(BlockedUserOut(id=target.id, name=target.first_name, blocked_at=b.created_at))
    return out
